...
import { isDirectory, listFilesRecursive, readFileIfExists, toScreamingSnakeCase } from './utils.js';
...
export function auditEnv(riff: string, repoRoot: string): EnvAuditResult {
	...
	const dotEnvLoaderPath = resolve(repoRoot, 'riffs', riff, 'src', 'lib', 'load-dotenv.ts');
