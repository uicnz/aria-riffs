...
export function auditSource(riff: string, repoRoot: string): SourceAuditResult {
	const srcDir = resolve(repoRoot, 'riffs', riff, 'src');
	const testDir = resolve(repoRoot, 'riffs', riff, 'test');
	const typesPath = resolve(repoRoot, 'riffs', riff, 'src', 'lib', 'types.ts');
	const typesDirPath = resolve(repoRoot, 'riffs', riff, 'src', 'lib', 'types');
	const schemaPath = resolve(repoRoot, 'riffs', riff, 'src', 'lib', 'schema.ts');
	const configLoaderPath = resolve(repoRoot, 'riffs', riff, 'src', 'lib', 'config.ts');
	...
	const directConsoleUsage = consoleBoundaryFiles.flatMap(file =>
		findConsoleBoundaryViolations(file, readFileSync(file, 'utf8'), resolve(repoRoot, 'riffs', riff))
	);
	const chalkUsageFiles = shippedRuntimeFiles
		.filter(file => usesChalk(readFileSync(file, 'utf8')))
		.map(file => relative(resolve(repoRoot, 'riffs', riff), file));
	...
	const nonBunRuntimeInvocations = shippedRuntimeFiles.flatMap(file =>
		findNonBunRuntimeInvocations(file, readFileSync(file, 'utf8'), resolve(repoRoot, 'riffs', riff))
	);
