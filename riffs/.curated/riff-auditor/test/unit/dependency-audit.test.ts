...
function writeSource(riff: string, source: string): void {
	const path = resolve(TEST_ROOT, 'riffs', '.curated', riff, 'src', 'cli.ts');
	mkdirSync(resolve(path, '..'), { recursive: true });
...
describe('Dependency Audit', () => {
	...
	it('given a standalone Bun package with aligned dependencies, when audited, then reports canonical dependencies', () => {
		writeRootManifest();
		writeJson(resolve(TEST_ROOT, 'riffs', '.curated', 'test-riff', 'package.json'), {
			packageManager: 'bun@1.4.2',
			engines: {
				bun: '>=1.4.0',
				node: '>=26.0.0',
	...
	it('given package identity and dependency drift, when audited, then reports every violation', () => {
		...
		writeJson(resolve(TEST_ROOT, 'riffs', '.curated', 'other-riff', 'package.json'), {
			dependencies: {
				commander: '15.0.0',
			},
		});
	...
	it('given invalid package JSON, when audited, then reports an invalid dependency contract', () => {
		writeRootManifest();
		const packagePath = resolve(TEST_ROOT, 'riffs', '.curated', 'test-riff', 'package.json');
