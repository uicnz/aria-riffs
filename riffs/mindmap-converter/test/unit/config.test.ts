...
describe('mindmap-converter Config', () => {
	beforeEach(() => {
		vi.resetModules();
		vi.clearAllMocks();
	});
	...
	describe('schema defaults', () => {
		it('given empty config, when loadConfig called, then returns valid config with correct types', async () => {
			...
			try {
				...
				expect(config).toHaveProperty('mindmap-converter');
				expect(config).toHaveProperty('logging');
				expect(typeof config['mindmap-converter']).toBe('object');
...
mindmap-converter:
    paths:
        output:
            dir: '~/.aria/output'
logging:
    file: '~/.aria/logs/mindmap-converter.log'
`
			);
	...
	describe('type contracts', () => {
		it('given loaded config, then required fields exist with correct types', async () => {
			...
			expect(config['mindmap-converter']).toBeDefined();
			expect(typeof config['mindmap-converter']).toBe('object');
