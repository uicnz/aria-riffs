...
import {
	extractSystemPrompt,
	extractUserMessage,
	findAndExtractUserMessage,
} from '../../src/core/content-extractor.js';
import type { Message, RequestBody } from '../../src/lib/types.js';
...
describe('content-extractor', () => {
	describe('extractUserMessage', () => {
		...
		it('returns empty string for invalid content type', () => {
			const message = {
				role: 'user',
				content: 123 as unknown, // number instead of string/array
			} as Message;
			expect(extractUserMessage(message)).toBe('');
		});
	});
